...
class Scraper(object):
	...
	def __init__(self, results_queue, browser_needed = True):
...
class FTDScraper(Scraper):
	def __init__(self, results_queue):
		super(FTDScraper, self).__init__(results_queue)
...
class FanduelScraper (Scraper):
	...
	def __init__(self, results_queue):
		super(FanduelScraper, self).__init__(results_queue)
	...
	def scrape(self):
		...
		for contest in contests[:1]:
			...
			contest_args = {
				...
				'start' : datetime.fromtimestamp(1388340000),
			}
			self.results_queue.put(Contest(**contest_args))

		# add sentinel when done
		self.results_queue.put(None) 
...
class DraftKingsScraper(Scraper):
	...
	def __init__(self, results_queue):
		super(DraftKingsScraper, self).__init__(results_queue, browser_needed = False)
